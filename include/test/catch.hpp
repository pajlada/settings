...
public:  // operators
    ...
    auto
    operator!=(StringRef other) const noexcept -> bool
    {
        return !(*this == other);
    }
...
public:  // named queries
    ...
    constexpr StringRef
    substr(size_type start, size_type length) const noexcept
    {
        if (start < m_size) {
            const auto shortened_size = m_size - start;
            return StringRef(m_start + start, (shortened_size < length)
                                                  ? shortened_size
                                                  : length);
        } else {
            return StringRef();
...
public:
    ...
    friend void
    swap(unique_ptr &lhs, unique_ptr &rhs)
    {
        auto temp = lhs.m_ptr;
        lhs.m_ptr = rhs.m_ptr;
        rhs.m_ptr = temp;
...
run_for_at_least(ClockDuration<Clock> how_long, const int initial_iterations,
                 Fun &&fun)
{
...
struct ObjectStorage {
    ...
    std::enable_if_t<AllowManualDestruction>
    destruct()
    {
        stored_object().~T();
    }
...
inline std::size_t
catch_strnlen(const char *str, std::size_t n)
{
...
class TestSpec
{
    ...
    private:
        ...
        friend std::ostream &
        operator<<(std::ostream &out, Pattern const &pattern)
        {
    ...
    struct Filter {
        ...
        friend std::ostream &
        operator<<(std::ostream &out, Filter const &f)
        {
            f.serializeTo(out);
            return out;
        }
...
private:
    ...
    void serializeTo(std::ostream &out) const;
    friend std::ostream &
    operator<<(std::ostream &out, TestSpec const &spec)
    {
...
public:
    ...
    Optional &
    operator=(Optional const &_other)
    {
        if (&_other != this) {
            reset();
            if (_other)
...
public:
    ...
    private:
        ...
        friend std::ostream &
        operator<<(std::ostream &lhs, ColourGuard &guard)
        {
...
struct ProcessedReporterSpec {
    ...
    friend bool
    operator!=(ProcessedReporterSpec const &lhs,
               ProcessedReporterSpec const &rhs)
    {
        return !(lhs == rhs);
    }
...
public:
    ...
    static auto
    logicError(std::string &&message) -> BasicResult
    {
        return {ResultType::LogicError, CATCH_MOVE(message)};
    }
    static auto
    runtimeError(std::string &&message) -> BasicResult
    {
    ...
    auto
    errorMessage() const -> std::string const &
    {
...
template <typename T>
ParserResult
convertInto(std::string const &source, T &target)
{
...
struct BoundValueRef : BoundValueRefBase {
    ...
    ParserResult
    setValue(std::string const &arg) override
    {
...
#define CATCH_INTERNAL_DEFINE_EXPRESSION_EQUALITY_OPERATOR(id, op)                          \
    ...
    {                                                                                       \
        return {static_cast<bool>(lhs.m_lhs op rhs), lhs.m_lhs, #op##_sr,                   \
                rhs};                                                                       \
    }                                                                                       \
    template <typename RhsT>                                                                \
    friend auto operator op(ExprLhs &&lhs, RhsT rhs)                                        \
        ...
        ->std::enable_if_t<                                                                 \
            Detail::conjunction<                                                            \
                ...
                Detail::is_eq_0_comparable<                                                 \
                    LhsT>, /* We allow long because we want `ptr op NULL` to be accepted */ \
    ...
    friend auto operator op(ExprLhs &&lhs, RhsT rhs)                                        \
        ->std::enable_if_t<                                                                 \
            Detail::conjunction<                                                            \
                Detail::negation<Detail::is_##id##_comparable<LhsT, RhsT>>,                 \
                Detail::is_eq_0_comparable<                                                 \
                    RhsT>, /* We allow long because we want `ptr op NULL` to be accepted */ \
...
#undef CATCH_INTERNAL_DEFINE_EXPRESSION_OPERATOR
    ...
    template <typename RhsT>
    friend auto
    operator&&(ExprLhs &&, RhsT &&) -> BinaryExpr<LhsT, RhsT const &>
    {
        static_assert(
...
struct Decomposer {
    template <
        ...
        std::enable_if_t<!std::is_arithmetic<std::remove_reference_t<T>>::value,
                         int> = 0>
    friend auto
    operator<=(Decomposer &&, T &&lhs) -> ExprLhs<T const &>
    {
    ...
    friend auto
    operator<=(Decomposer &&, T value) -> ExprLhs<T>
    {
...
#define CATCH_AND_THEN(desc) \
    ...
    defined(                              \
        CATCH_CONFIG_DISABLE)  // ^^ prefixed, implemented | vv prefixed, disabled
...
#elif !defined(CATCH_CONFIG_PREFIX_ALL) && \
    !defined(                              \
        CATCH_CONFIG_DISABLE)  // ^^ prefixed, disabled | vv unprefixed, implemented
...
#elif !defined(CATCH_CONFIG_PREFIX_ALL) && \
    defined(                               \
        CATCH_CONFIG_DISABLE)  // ^^ unprefixed, implemented | vv unprefixed, disabled
...
class Generators : public IGenerator<T>
{
    ...
    void
    add_generator(GeneratorWrapper<T> &&generator)
    {
        m_generators.emplace_back(CATCH_MOVE(generator));
    }
    void
    add_generator(T const &val)
    {
    ...
    template <typename U>
    void
    add_generators(U &&valueOrGenerator)
    {
    ...
    void
    add_generators(U &&valueOrGenerator, Gs &&...moreGenerators)
    {
        add_generator(CATCH_FORWARD(valueOrGenerator));
...
template <typename T, typename... Gs>
auto
makeGenerators(T &&val, Gs &&...moreGenerators) -> Generators<std::decay_t<T>>
{
...
random(T a, T b)
{
    static_assert(!std::is_same<T, char>::value &&
                  ...
                  "The requested type is not supported by the underlying "
                  "random distributions from std");
    return GeneratorWrapper<T>(
...
template <typename Container>
constexpr auto
empty(Container const &cont) -> decltype(cont.empty())
{
    return cont.empty();
}
template <typename T, std::size_t N>
constexpr bool
empty(const T (&)[N]) noexcept
{
...
template <typename T>
constexpr bool
empty(std::initializer_list<T> list) noexcept
{
    return list.size() > 0;
}
...
template <typename Container>
constexpr auto
size(Container const &cont) -> decltype(cont.size())
{
    return cont.size();
}
template <typename T, std::size_t N>
...
struct NameAndLocationRef {
    ...
    friend bool
    operator==(NameAndLocation const &lhs, NameAndLocationRef rhs)
    {
        return StringRef(lhs.name) == rhs.name && lhs.location == rhs.location;
    }
    friend bool
    operator==(NameAndLocationRef lhs, NameAndLocation const &rhs)
    {
        return rhs == lhs;
    }
};
...
template <typename Container>
Container
createShard(Container const &container, std::size_t const shardCount,
...
public:
    ...
    Column &
    indent(size_t newIndent)
    {
        m_indent = newIndent;
        return *this;
    }
    Column &
    initialIndent(size_t newIndent)
    {
...
template <typename ArgT, typename MatcherT>
auto
makeMatchExpr(ArgT &&arg, MatcherT const &matcher) -> MatchExpr<ArgT, MatcherT>
{
...
public:
    ...
    friend MatchAllOf
    operator&&(MatchAllOf &&lhs, MatcherBase<ArgT> const &rhs)
    {
        lhs.m_matchers.push_back(&rhs);
        return CATCH_MOVE(lhs);
    }
    friend MatchAllOf
    operator&&(MatcherBase<ArgT> const &lhs, MatchAllOf &&rhs)
    {
...
public:
    ...
    friend MatchAnyOf
    operator||(MatchAnyOf &&lhs, MatcherBase<ArgT> const &rhs)
    {
        lhs.m_matchers.push_back(&rhs);
        return CATCH_MOVE(lhs);
    }
    friend MatchAnyOf
    operator||(MatcherBase<ArgT> const &lhs, MatchAnyOf &&rhs)
    {
        rhs.m_matchers.insert(rhs.m_matchers.begin(), &lhs);
        return CATCH_MOVE(rhs);
    }
...
template <typename T>
Detail::MatchAllOf<T>
operator&&(MatcherBase<T> const &lhs, MatcherBase<T> const &rhs)
{
...
template <typename T>
Detail::MatchNotOf<T>
operator!(MatcherBase<T> const &matcher)
{
...
bool
match_all_of(Arg &&, std::array<void const *, N> const &, std::index_sequence<>)
{
    return true;
}
...
bool
match_all_of(Arg &&arg, std::array<void const *, N> const &matchers,
...
template <std::size_t N, typename Arg>
bool
match_any_of(Arg &&, std::array<void const *, N> const &, std::index_sequence<>)
{
    return false;
}
...
template <typename T, typename... MatcherTs, std::size_t N, typename Arg,
          std::size_t Idx, std::size_t... Indices>
bool
match_any_of(Arg &&arg, std::array<void const *, N> const &matchers,
             std::index_sequence<Idx, Indices...>)
{
    return static_cast<T const *>(matchers[Idx])->match(arg) ||
...
/**
         ...
         */
template <typename T, typename Equality>
ContainsElementMatcher<T, Equality>
...
public:
    ...
    bool
    match(std::exception const &ex) const override
    {
...
Predicate(Pred &&predicate, std::string const &description = "")
{
...
template <typename Matcher>
AllMatchMatcher<Matcher>
AllMatch(Matcher &&matcher)
{
    return {CATCH_FORWARD(matcher)};
}
...
public:
    ...
    match(RangeLike &&rng) const
    {
        using std::begin;
        using std::end;
        return std::equal(begin(m_desired), end(m_desired), begin(rng),
                          end(rng), m_predicate);
    }
...
public:
    ...
    match(RangeLike &&rng) const
    {
        using std::begin;
        using std::end;
        return std::is_permutation(begin(m_desired), end(m_desired), begin(rng),
                                   end(rng), m_predicate);
    }
...
public:
    ...
    void
    benchmarkPreparing(StringRef) override
    {
    }
    void
    benchmarkStarting(BenchmarkInfo const &) override
    {
    }
    void
    benchmarkEnded(BenchmarkStats<> const &) override
    {
    }
    void
    benchmarkFailed(StringRef) override
    {
    }

    void
    fatalErrorEncountered(StringRef /*error*/) override
    {
    }
    void
    noMatchingTestCases(StringRef /*unmatchedSpec*/) override
    {
    }
    void
    reportInvalidTestSpec(StringRef /*invalidArgument*/) override
    {
    }
    ...
    void
    testCasePartialStarting(TestCaseInfo const &, uint64_t) override
    {
    }
    void
    ...
    void
    testCasePartialEnded(TestCaseStats const &, uint64_t) override
    {
    }
    void
...
public:
    ...
    void
    noMatchingTestCases(StringRef) override
    {
    }
    void
    reportInvalidTestSpec(StringRef) override
    {
    }
    void
    fatalErrorEncountered(StringRef /*error*/) override
...
{
    IEventListenerPtr
    create(ReporterConfig &&config) const override
    {
...
class ListenerRegistrar
{
    class TypedListenerFactory : public EventListenerFactory
    {
        ...
        std::string
        getDescriptionImpl(std::true_type) const
        {
            return T::getDescription();
        }

        std::string
        getDescriptionImpl(std::false_type) const
        {
